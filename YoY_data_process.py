...
def process_data(data_version):
    ...
    for written_file_count in range( int(len(to_file)/100 + 1) ): # YoY has limits on input size. From experience, the 200,000 amino acids limit gets hit with some 250 proteins
        with open(os.path.join(dir_path, f'YoY_file_{data_version}_{written_file_count}.txt'), 'w') as f:
            for line_count, line in enumerate(to_file[written_file_count*250 : (written_file_count+1)*250]):
                f.writelines(line)
...
def process_results(data_version):
    ...
    for idx, line in enumerate(lines):
        if '+' in line: # Technically should also check for -, but all sites are +++ or ++++
    ...
    if data_version == 'v3':
        ...
        raise NotImplementedError('No analysis has been carried on data_versions != "v3"')
    ...
    for idx in range(real_data.shape[0]):
        temp = np.where((thresholds.iloc[:, 0] == real_data.iloc[idx, 0]) & (numerical_yoy_position == real_data.iloc[idx, 1]))[0]
        if np.any(temp):
